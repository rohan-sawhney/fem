...
Index parseFaceIndex(const string& token)
{
...
string stringRep(const Vector3d& v)
{
...
void MeshIO::preallocateMeshElements(const MeshData& data, Mesh& mesh)
{
    ...
    size_t nHE = 2*nE;
    size_t chi = nV - nE + nF;
    int nB = max(0, 2 - (int)chi); // conservative approximation of number of boundary cycles
...
{
    unordered_map<string, int> vertexFaceMap;
...
bool MeshIO::buildMesh(const MeshData& data, Mesh& mesh)
{
    ...
    for (vector<vector<Index>>::const_iterator f  = data.indices.begin();
        ...
        if (n < 3) {
            cerr << "Error: face " << faceIndex << " is degenerate" << endl;
        ...
        vector<HalfEdgeIter> halfEdges(n);
        for (int i = 0; i < n; i++) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (existingHalfEdges.find(pair<int, int>(a, b)) != existingHalfEdges.end()) {
                halfEdges[i]->flip = existingHalfEdges[pair<int, int>(a, b)];
            ...
            existingHalfEdges[pair<int, int>(a, b)] = halfEdges[i];
            
            // check for nonmanifold edges
            edgeCount[pair<int, int>(a, b)] ++;
            if (edgeCount[pair<int, int>(a, b)] > 2) {
                cerr << "Error: edge " << a << ", " << b << " is non manifold" << endl;
                return false;
            }
        }
...
bool MeshIO::read(ifstream& in, Mesh& mesh)
{
    MeshData data;
    ...
    while(getline(in, line)) {
        ...
        if (token == "v") {
            ...
            data.positions.push_back(Vector3d(x, y, z));
            
        } else if (token == "vt") {
            double u, v;
            ss >> u >> v;
            
            data.uvs.push_back(Vector3d(u,v,0));
            
        } else if (token == "vn") {
            double x, y, z;
            ss >> x >> y >> z;
            
            data.normals.push_back(Vector3d(x, y, z));
...
void MeshIO::write(ofstream& out, const Mesh& mesh)
{
