...
void GLMesh::fillBuffers(const vector<Vector3f>& colors)
{
    ...
    for (FaceCIter f = mesh.faces.begin(); f != mesh.faces.end(); f++) {
        if (!f->isBoundary()) {
            ...
            // set barycenters
            vertices[fIdx+0].barycenter = Vector3f(1.0, 0.0, 0.0);
            vertices[fIdx+1].barycenter = Vector3f(0.0, 1.0, 0.0);
            vertices[fIdx+2].barycenter = Vector3f(0.0, 0.0, 1.0);
        }
    }
}

Vector3f elementColor(unsigned int idx)
{
    return Vector3f(((idx & 0x000000FF) >>  0)/255.0,
...
void GLMesh::fillPickBuffers()
{
    ...
    for (FaceCIter f = mesh.faces.begin(); f != mesh.faces.end(); f++) {
        if (!f->isBoundary()) {
            int j = 0;
            Vector3f color = elementColor(f->index);
    ...
    for (VertexCIter v = mesh.vertices.begin(); v != mesh.vertices.end(); v++) {
        Vector3d n = 0.0015*v->normal();
        Vector3f p1 = (v->position + n).cast<float>();
        Vector3f color = elementColor(tris + v->index);
        
        HalfEdgeCIter h = v->he;
        do {
            if (!h->onBoundary) {
                Vector3f p2 = (h->next->vertex->position + n).cast<float>();
                Vector3f p3 = (h->next->next->vertex->position + n).cast<float>();
...
void GLMesh::setup(const vector<Vector3f>& colors)
{
    fillBuffers(colors);
    fillPickBuffers();
...
void GLMesh::update(const vector<Vector3f>& colors)
{
    fillBuffers(colors);
    fillPickBuffers();
